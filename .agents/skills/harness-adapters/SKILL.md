...
**Standing operating rule until that confirmation lands:** every `cursor` crewmate/secondmate dispatch - static `config/crew-harness`, `config/secondmate-harness`, and every `config/crew-dispatch.json` profile - MUST use `--model auto` (or omit `--model`) and MUST NOT select a named model or a specific effort level for `cursor`.
...
The launch template is kind-agnostic (identical command for ship/scout/secondmate, matching grok's shape) and cursor-agent needs no per-kind extension files the way pi/codex do, so the `--secondmate` LAUNCH falls out of the same code path - but the secondmate path itself (home provisioning, registry validation, inherited config) was not independently spawned or observed for this adapter.
One piece of separate wiring was required: a secondmate runs its own primary firstmate session, so it must acquire its own home's session lock, and `bin/fm-session-lock-lib.sh`'s harness identity keys on `ps -o comm=`, which for cursor-agent is `MainThread` and matches no `FM_HARNESS_RE` alternative.
That file therefore carries the same dedicated `MainThread` + `cursor-agent`-in-argv identity `bin/fm-harness.sh` uses (`fm_harness_is_cursor`, used by both the ancestry walk and the holder-liveness check); without it a cursor secondmate would fail lock acquisition and degrade to a permanently READ-ONLY session.
