...
In particular the fallback is NOT the last-used model, despite how it first read - every observed drop or substitution landed on `Gemini 3.6 Flash (High)`, including the probe immediately after a launch that had run `gemini-3.6-flash-low` in the same HOME (which would have read `(Low)` if the fallback were sticky).
