...
`bin/fm-spawn.sh` refuses a `--secondmate` spawn with `harness=cursor` outright (both via `config/secondmate-harness` and an explicit `--harness cursor` override), before any endpoint or worktree is created, with a diagnostic naming the reason.
...
Regression coverage: `tests/fm-secondmate-harness.test.sh`'s `test_spawn_cursor_secondmate_refused` pins the spawn-time refusal (both resolution paths); `tests/fm-cursor-harness.test.sh`'s `test_session_lock_has_no_cursor_identity` pins the session-lock absence.
