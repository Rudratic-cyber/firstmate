...
| antigravity | `--model <model>`, emitted before `-i` | none (exists but unsafe) | Verified 2026-08-02 on agy 1.1.9. `-i` is a valued flag (`--prompt-interactive`), NOT a positional argument, so order relative to it is not load-bearing: `--model` placed after `-i "<prompt>"` was verified live to be honored. The shape that silently drops `--model` is a bare positional prompt with no `-i` (`agy ... '<prompt>' --model <model>`); the model such a session falls back to was not root-caused and is not the last-used one (see the Antigravity section below). fm-spawn emits `--model` before `-i`, the placement verified to win under every shape observed. agy exposes a real `--effort low\|medium\|high` flag, unlike cursor, but it was verified live to silently corrupt `--model` resolution when both are passed together: `--model claude-sonnet-4-6 --effort medium` dropped the requested model to agy's default Gemini with no error. Firstmate resolves the intended effort into the chosen `--model` value at intake instead, exactly like cursor - but that fold-effort-into-the-id strategy is itself verified-unreliable for at least one advertised id: `--model gemini-3.5-flash-low` alone, with no `--effort` in the command, runs as `Gemini 3.5 Flash (Medium)`. Treat an id's effort tier as unverified until it is confirmed live from the session's own banner - see the Antigravity section below. |
...
| antigravity | Run `agy models`, which lists the model ids agy advertises for the current account (verified live: includes `gemini-3.6-flash-high\|medium\|low`, `gemini-3.5-flash-high\|medium\|low`, `gemini-3.1-pro-high\|low`, `claude-sonnet-4-6`, `claude-opus-4-6-thinking`, `gpt-oss-120b-medium`). **That listing is NOT proof the id you pass is the model that runs: agy silently substitutes.** Verified live, with the immediately preceding launch in the same HOME run on Claude so a Gemini reading is unambiguous, `agy --dangerously-skip-permissions --new-project --model gemini-3.1-pro-high -i '...'` started a session whose own banner read `Gemini 3.6 Flash (High)` - agy accepted an id from its own `agy models` listing, then ran a completely different model, with no error and nothing in the rendered output indicating a substitution happened. Contrast cursor-agent one row above, which refuses loudly and names the exact rejected model (`ActionRequiredError: Named models unavailable ...`): a loud refusal is recoverable, a quiet substitution is not, and firstmate's dispatch relies on this listing being trustworthy, so this is precisely the failure class this skill exists to prevent. Nothing between `config/crew-dispatch.json` and the launched session catches it - `crew_dispatch_validate` accepts any string as `model`, and `model_flag_for_harness` threads it verbatim. Substitution is not confined to one family: `--model gemini-3.5-flash-low`, with no `--effort` in the command, runs as `Gemini 3.5 Flash (Medium)` (reproduced 3x) - the right model at an unrequested effort tier, which also makes the adapter's fold-effort-into-the-model-id strategy unreliable for that id. 6 of the 11 listed ids were exercised: 3 honored (`claude-sonnet-4-6`, `gemini-3.6-flash-low`, `gemini-3.5-flash-high`), 2 silently substituted (`gemini-3.1-pro-high` -> `Gemini 3.6 Flash (High)`, `gemini-3.5-flash-low` -> `Gemini 3.5 Flash (Medium)`), and 1 indeterminate (`gemini-3.6-flash-high`, whose banner is identical to the observed fallback target, so it cannot discriminate honored from substituted); the remaining 5 are untested. It is unknown what governs which ids are substituted, and unknown whether this is specific to this account's tier. Treat every id whose running identity you have not confirmed live from the session's own banner as unverified - including `gemini-3.1-pro-high` and `gemini-3.5-flash-low`, which are confirmed NOT to run what they name. |
...
### Cursor and antigravity never report an empty composer, so every send to them exits 1

Both adapters are in the verified-dispatch set (`bin/fm-bootstrap.sh`'s `crew_dispatch_validate`, the `--secondmate cursor|antigravity` token in `bin/fm-spawn.sh`), which makes them reachable by firstmate's automated send paths.
Neither one's composer can EVER classify `empty`, and each for its own mechanism:
- **cursor**: the real terminal cursor (`#{cursor_y}`) is observed parked outside the `▄`/`▀` rule pair, so `fm_tmux_find_composer_box`'s half-block guard forces `unknown` - both when the composer is genuinely idle and when it holds unsubmitted text.
- **antigravity**: the composer is a bare `>` outside any bordered box, which `fm_composer_classify_content`'s dead-shell-prompt rule returns as `unknown` (never `empty`) by design.

Because `empty` is the only verdict `bin/fm-send.sh` accepts as proof of delivery, EVERY `fm-send.sh` to a live cursor or antigravity pane takes the non-`empty` arm and exits 1 with "delivery unconfirmed" - including the sends that actually landed.
This is fail-safe in direction (never a false success) but it is not inert, because two automated paths act on that exit code and both are wrong on the success case:
- **The pending-reply correlation record is destroyed.** `bin/fm-send.sh:312` (the `*)` verdict arm) calls `fm_pending_reply_discard_undelivered` whenever `PENDING_REPLY_CREATED=1`, which is set for any send to a `kind=secondmate` target. The message landed, but the parent's correlation record is gone, so recovery can never fire and the worker's correlated reply cannot resolve against anything - firstmate silently forgets it was waiting.
- **A config push will re-deliver itself.** `fm_config_reread_send_pointer` (`bin/fm-config-inherit-lib.sh`) only removes its `.pending` marker when `fm-send` exits 0, so the false failure leaves the retry marker in place and the next push re-sends the identical `CONFIG_REREAD:` instruction to a still-running agent. That is exactly the "do not treat that non-zero exit as a swallow and resend" warning below, executed automatically rather than by a human operator - so read that warning as addressed to both readers, and expect the config-reread path to violate it until the send contract itself changes.

Gating these adapters out of the correlated-send paths, or teaching `fm-send` a per-harness "delivery cannot be proven" contract, is a change to SHARED send semantics that affects every existing adapter; it is queued separately and deliberately not attempted here.
Until then: for cursor and antigravity, treat an `fm-send` exit 1 as carrying no information about delivery, verify the pane directly with `fm-peek`, and do not resend on the strength of the exit code alone.
...
The consequence for `fm-send` is NOT a retry, and this is the part to read carefully: `fm_tmux_submit_enter_core` (`bin/fm-tmux-lib.sh`) continues its Enter-retry loop ONLY while the verdict is `pending` or `pending-unproven`, and returns immediately on any other verdict.
Because the fixed detector returns `unknown` for a Cursor pane in BOTH the idle and the genuinely-pending case, a Cursor send sends exactly ONE Enter and returns `unknown` - the Enter-retry budget is never spent, so the second-Enter free-text hazard documented above is NOT mitigated by it.
`fm-send` then takes its non-`empty` arm and exits 1 with "delivery unconfirmed" (`bin/fm-send.sh`), which is fail-safe in direction (never a false success) but is reported identically whether the text actually landed or is still sitting unsubmitted in the composer.
Because that `unknown` is returned for the idle composer too, a Cursor pane can never confirm a delivery at all - see "Cursor and antigravity never report an empty composer, so every send to them exits 1" above for the two automated paths that act on the resulting exit 1.
...
Launch with an initial prompt passed to the `-i` flag: `agy --dangerously-skip-permissions --new-project --model <model> -i "$(cat <brief>)"`.
...
| Fact | Value |
|---|---|
| Launch | Bare interactive TUI with an initial prompt passed to the `-i`/`--prompt-interactive` valued flag, `--dangerously-skip-permissions --new-project --model <model>`; the prompt auto-runs once autonomy is granted - verified the same shape as claude/grok, NOT Kimi's launch-then-send (a multi-line prompt was also verified to run correctly as one message). |
...
| Effort | A real `--effort low\|medium\|high` flag exists but is unsafe to combine with `--model` - see the launch-profile-axes table above and the `--model` note below. |
| Model discovery | `agy models`; see the model-discovery table above, including the verified silent-substitution warning - a listed id is not proof that id is what runs. |
| Account observed | `info.rudratic@gmail.com (Google AI Pro)` - a real paid tier, confirmed live from the TUI's own startup banner. `--model claude-sonnet-4-6` produced a genuine model response with a real extended-thinking trace, not a plan-tier rejection - antigravity does not carry Cursor's "verified-except-model-tier" caveat. `--model gemini-3.6-flash-high` also produced a real response, but that banner is identical to the observed silent-substitution fallback (see the model-discovery table above), so it cannot discriminate honored from substituted and is NOT evidence that id is honored. |
...
**`--model` is dropped after a bare positional prompt, and `--effort` must never be passed at all (confirmed 2026-08-02).**
`-i` is a valued flag (`--prompt-interactive`, per `agy --help`: "Short alias for `--prompt-interactive`"), NOT a positional argument, so flag order relative to `-i` is not the mechanism.
Verified live in both directions, in one shared isolated HOME, with the immediately preceding launch run on the OTHER model family so the banner reading is unambiguous: `agy --dangerously-skip-permissions --new-project -i 'say hi' --model gemini-3.6-flash-low` ran as `Gemini 3.6 Flash (Low)` and the reverse probe ran as `Claude Sonnet 4.6` - the flag placed AFTER `-i "<prompt>"` was honored each time.
The shape that really does drop the flag is a genuinely bare positional prompt with no `-i` at all: `agy --dangerously-skip-permissions --new-project 'say hi' --model claude-sonnet-4-6` ran as `Gemini 3.6 Flash (High)`, silently discarding the requested model.
Neither half of that behavior was root-caused, so record both as observations and do not build on either: the parsing reason a trailing `--model` is dropped after a bare positional is unknown, and so is the fallback target.
In particular the fallback is NOT the last-used model, despite how it first read - every observed bare-positional DROP landed on `Gemini 3.6 Flash (High)` regardless of what the preceding launch in the same HOME had set, confirmed against three different predecessors (`gemini-3.6-flash-low`, `claude-sonnet-4-6`, `gemini-3.5-flash-low`), any of which would have produced a different banner if the fallback were sticky.
That fixed target is a claim about the drop paths only; the silent substitutions on the `--model`-is-honored path do NOT all land there (`gemini-3.5-flash-low` lands on `Gemini 3.5 Flash (Medium)` - see the model-discovery table above).
Do not conclude from any of this that agy's flag parsing is order-sensitive for valued flags generally.
`fm-spawn.sh`'s launch template places `__MODELFLAG__` before `-i`, the placement verified to win under every shape observed, and that placement does not need to change; `tests/fm-spawn-dispatch-profile.test.sh` pins it.
Separately, `--effort` (which DOES exist as a real flag, unlike cursor) was verified to corrupt `--model` resolution even when correctly ordered: `--model claude-sonnet-4-6 --effort medium` silently fell back to the default Gemini model with no error, while `claude-sonnet-4-6` on its own is honored every time.
The supported statement is exactly that - adding `--effort` dropped the requested model to the default; the mechanism is unknown.
Do NOT record it as `--effort` overriding a model's own baked-in effort suffix: that characterization was derived solely from `--model gemini-3.5-flash-low --effort low` running as `Gemini 3.5 Flash (Medium)`, and the identical command with `--effort` REMOVED produces the identical `(Medium)` banner (reproduced 3x), so that case is an id-level silent substitution that has nothing to do with `--effort` (see the model-discovery table above).
...
That decision stands on the surviving `claude-sonnet-4-6` case and is unaffected, but note the fold-into-the-id strategy is not itself proven for every id: `gemini-3.5-flash-low` silently delivers Medium effort to a captain who picked it to get Low.
...
Because that safety rests entirely on a shared rule agy does not own, those two observed shapes are reproduced as regression fixtures: `test_antigravity_bare_prompt_glyph_idle_is_unknown` and `test_antigravity_bare_prompt_glyph_with_text_is_pending` in `tests/fm-composer-ghost.test.sh`.
They exist to fail loudly if the dead-shell-prompt rule is ever "refined" to treat a bare `>` the way it already treats the sibling glyphs `❯`/`›` (which return `empty`) - that would make every idle agy pane read proven-empty, giving `fm-send` a false delivery confirmation and handing the away-mode injector a pane that may hold unsubmitted text.
The supervision consequence matches Cursor's - `fm_pane_input_pending` treats the never-proven-empty idle composer as pending and defers rather than injecting - but the `fm-send` consequence does NOT.
Unlike Cursor, agy's `cursor_y` tracks the composer row, so genuinely unsubmitted text classifies as proven `pending` and `fm_tmux_submit_enter_core` really does spend its Enter-retry budget on it (Cursor's `unknown` exits that loop on the first Enter).
The cost is on the success path instead: once the Enter lands, agy's composer returns to a bare `>` and reads `unknown`, not `empty`, so `fm-send` takes its non-`empty` arm and exits 1 with "delivery unconfirmed" (`bin/fm-send.sh`) after having actually delivered.
Do not treat that non-zero exit as a swallow and resend: the composer really did clear, so a resend types into an empty composer and submits cleanly, delivering a second copy of the same instruction to a running agent.
That warning currently binds humans only - `fm_config_reread_send_pointer`'s retry marker does exactly the resend it forbids, and the same false failure discards the pending-reply correlation record for secondmate targets; see "Cursor and antigravity never report an empty composer, so every send to them exits 1" above for both consequences and the shared-send-semantics change queued to fix them.
