...
**Account/plan caveat (2026-08-02, needs a captain decision before treating model selection as fully verified).**
...
What is NOT verified end to end: that a real named/effort-bearing model (the entire point of folding effort into the model id) actually completes a turn on this installed CLI and account.
Before firstmate dispatches a `cursor` crewmate with a non-`auto` `--model` in production, either confirm the CLI is authenticated to the captain's actual Pro account and re-run a real named-model turn, or keep dispatch profiles for `cursor` on `auto` until that confirmation lands.
...
Consequence for supervision: `fm_pane_input_pending` treats anything not proven `empty` as pending and defers (safe - the away-mode injector will not treat a Cursor pane as a confirmed injection target), and `fm-send`'s submit-retry core falls back through its Enter-retry budget before returning a verdict, which is very likely to land the needed second Enter for the free-text case above but can still report non-zero ("swallow" rather than a false success) rather than a clean confirmation.
...
Regression coverage: `tests/fm-cursor-harness.test.sh`.

**Secondmate note.** Only a crewmate (ship/scout) launch was spawned and observed end to end.
The launch template is kind-agnostic (identical command for ship/scout/secondmate, matching grok's shape) and cursor-agent needs no per-kind extension files the way pi/codex do, so `--secondmate` dispatch falls out of the same code path with no separate wiring - but the secondmate path itself (home provisioning, registry validation, inherited config) was not independently spawned or observed for this adapter.
