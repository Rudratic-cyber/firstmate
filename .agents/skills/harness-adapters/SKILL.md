...
Regression coverage: `tests/fm-composer-ghost.test.sh` (both half-block fixtures plus the no-crossover case for bordered harnesses); `tests/fm-cursor-harness.test.sh` covers harness detection, not the composer.
