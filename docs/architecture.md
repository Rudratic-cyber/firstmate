...
Codex, standalone Kimi, cursor, and antigravity classify unknown behind explicit probes until a semantic source is live-verified for them, and Grok keeps one clearly isolated rendered-tail fallback that can only ever classify a Grok task.
...
That keeps spawn launch compatible across claude, codex, grok, pi, opencode, kimi, cursor, and antigravity while preserving the requested profile for later audit.
