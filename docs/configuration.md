...
cursor and antigravity are verified for crewmate launches, with the secondmate path unverified for both (cursor is refused outright as a secondmate harness; antigravity's secondmate path is merely unexercised), and each carries a model-selection limit the harness-adapters skill below owns the evidence for: every cursor dispatch stays pinned to `--model auto` until a named-model turn is confirmed end to end, and an id listed by `agy models` is not proof antigravity runs that model.
