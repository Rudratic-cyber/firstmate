...
antigravity is verified for crewmate and scout launches; its `--secondmate` path was not independently exercised (the launch template is kind-agnostic and needs no per-kind wiring, so it is expected to work, but is unproven) - the harness-adapters skill below owns the evidence.
