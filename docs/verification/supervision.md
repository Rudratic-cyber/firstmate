...
| Cursor | cursor-agent 2026.07.23-e383d2b, probed 2026-08-02 | None usable | No lifecycle-hook or event-stream mechanism the supervised interactive TUI can expose, so the gate stays closed and it classifies `unknown cursor-unverified`; the `harness-adapters` skill's Cursor section owns the probe detail. |
