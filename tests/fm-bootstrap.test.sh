...
cursor model profile is accepted^{"rules":[{"when":"cursor work","use":{"harness":"cursor","model":"auto"}}]}^empty^
unsupported cursor effort is flagged^{"rules":[{"when":"cursor work","use":{"harness":"cursor","model":"auto","effort":"high"}}]}^exact^CREW_DISPATCH: invalid config/crew-dispatch.json - invalid effort: cursor:high
