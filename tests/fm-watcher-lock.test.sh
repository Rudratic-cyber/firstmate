...
test_guard_warnings() {
  ...
  grep -F 'automatic watcher mechanism is broken' "$err" >/dev/null || fail "guard banner missing the automatic-mechanism failure"
  grep -F 'queued wakes pending - drain them' "$err" >/dev/null || fail "guard did not warn about pending queue"
  grep -F 'After draining queued wakes, the Stop-owned automatic watcher mechanism is broken' "$err" >/dev/null || fail "guard did not order automatic failure handling after drain"
